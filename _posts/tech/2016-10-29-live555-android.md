...
其中LOCAL_SRC_FILES的下面的一堆值可能会有问题，根据版本的不同类名也会有增删变化，所以如果不行，可以用下面代码自己生成，然后将生成的文件内容替换LOCAL_SRC_FILES的值:
	...
	public static void main(String[] args) {
		File file =  new File("....app/src/main/jni");
		File outFile = new File("....app/src/main/jni/output.txt");
		try {
			...
			for(File f : files){
				if(f.isDirectory()){
					...
					for(File f2:files2){
						outputPath = f.getName();
						if(f2.getName().endsWith(".cpp") || f2.getName().endsWith(".c")){
							outputPath += "/" + f2.getName() + "  \\\r\n"; 
							ps.append(outputPath);
							outputPath = "";
						}
					}
				}
